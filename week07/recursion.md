...
Тук на параметъра "key" на метода sort подаваме като аргумент ламбда функция, която при вход "x" — наредена двойка — взима втория елемент и го връща като резултат. По този начин сортираме листа от редици по втори елемент.

## Следваща стъпка

[Решете задачите за рекурсия](recursion_tasks.md) · [Проверете решенията си с `assert` и pytest](testing.md)

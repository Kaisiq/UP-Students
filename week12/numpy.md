...
`np.intersect1d(a, b)` връща едномерен масив с уникалните общи елементи на `a` и `b`:

```py
a = np.array([1, 2, 2, 5])
b = np.array([2, 3, 5])
print(np.intersect1d(a, b))  # [2 5]
```

### ж) Boolean masking

Булевата маска избира само елементите, които изпълняват условие.

```py
matrix = np.array([[1, -3], [8, 12]])
selected = matrix[(matrix >= -2) & (matrix <= 10)]
print(selected)           # [1 8]
```

### з) Превръщане в един размер
```py
matrix = np.array([[1, -3], [8, 12]])
print(matrix.ravel())     # [ 1 -3  8 12]
```
> `ravel()` връща едномерен изглед, когато е възможно; `flatten()` винаги прави копие.

### и) Случайни цели числа

```py
secret = np.random.randint(0, 10, size=4)
```

## Следваща стъпка

[Решете задачите за NumPy](numpy_tasks.md) · [Научете работа с файлове](files.md)

...
* Грешките по време на изпълнение се наричат изключения (Exceptions). Синтактичните грешки се откриват преди изпълнението и не могат да бъдат обработени с `try`/`except` в същия код.
